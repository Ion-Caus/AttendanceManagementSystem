...
public class LessonDataDAOImpl implements LessonDataDAO
{
      ...
      throws SQLException   // TODO check! not 100% sure
  {
    ...
    {
      PreparedStatement statement = connection.prepareStatement(
          "SELECT * FROM lesson_data join schedule_lessons sl on lesson_data.lessonid = sl.lessonid WHERE userid = ? and lessonid = ?");
      statement.setString(1, studentID);
  ...
  @Override public ArrayList<LessonData> readAllByStudentID(String studentID)
      throws SQLException
  {
    try (Connection connection = getConnection())
    {
      PreparedStatement statement = connection
          .prepareStatement("SELECT * FROM lesson_data ld join user_account ua on ua.userid = ld.userid " +
                  "join schedule_lessons sl on ld.lessonid = sl.lessonid WHERE ld.userid = ?");
      return getLessonData(studentID, statement);
    }
  }

  @Override public ArrayList<LessonData> readAllByLessonID(String lessonID)
      throws SQLException
  {
    try (Connection connection = getConnection())
    {
      PreparedStatement statement = connection
          .prepareStatement("SELECT * FROM lesson_data join user_account ua on ua.userid = lesson_data.userid " +
                  "join schedule_lessons sl on lesson_data.lessonid = sl.lessonid WHERE lessonid = ?");
      ...
      throws SQLException
  {
    ...
    Lesson lesson = new Lesson(id, teacher, new Date(date), new Time(timeFrom),
        new Time(timeTo), subject, topic, contents, classroom, homework,className);
